...
import configData from "../../app/modules/services/config.json";
...
export function checkHasAccessReports(userRoles) {
    let hasAccess = false,
        accessRoles = configData.REPORT_ACCESS_ROLES;

    if(userRoles.length > 0) {
        hasAccess = !!userRoles.filter((item) => { 
            return accessRoles.indexOf(item) !== -1 
        });
    }
    return hasAccess;
};
